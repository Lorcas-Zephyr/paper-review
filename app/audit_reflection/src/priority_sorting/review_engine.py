...
# 与 orchestrator / conflict_resolver 中 group_id 含义一致（字符串 group_id 也能解析）
_GROUP_ID_FALLBACK_NAMES: Dict[int, str] = {
    2: "格式审计组",
    3: "逻辑审计组",
    4: "代码审计组",
    5: "实验数据审计组",
...
def _parse_group_id_int(group_id: Any) -> Optional[int]:
...
class ReviewDecisionEngine:
    ...
    def process_audit_results(self, raw_audit_list: List[Dict[str, Any]]) -> Tuple[
        ...
        for raw_group in raw_audit_list:
            ...
            explicit_gn = (raw_group.get("group_name") or "").strip()
            if explicit_gn:
                group_name = explicit_gn
            else:
                gid_int = _parse_group_id_int(group_id)
                group_name = _GROUP_ID_FALLBACK_NAMES.get(gid_int, f"审计组{group_id}")

            # 处理audit_results数组
            audit_results = raw_group.get("audit_results", [])
            gid_key = str(group_id).strip().replace(" ", "")
            for j, raw_data in enumerate(audit_results):
                # 添加group信息到每条结果
                raw_data["audit_agent"] = group_name
                base = (raw_data.get("result_id") or raw_data.get("id") or "").strip()
                # 全空或重复的 id 会导致反思阶段按 result_id 回填 description 时错配到其他组条目
                synthetic = f"g{gid_key}-i{j}" + (f"-{base}" if base else "")
                raw_data["result_id"] = synthetic
                raw_data["id"] = synthetic
